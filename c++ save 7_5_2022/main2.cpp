...
enum states{IDLE, GLIDING, WALKING, RUNNING, SWIMMING};
...
struct Entity {
    ...
    void render(std::array<double, 2> camera_pos, float scale, unsigned int shader, std::array<int, 2> window_size) {
        draw_tile(shader, spritesheet, {float((position[0] - camera_pos[0] + visual_offset[0]) * scale), float((position[1] - camera_pos[1] + visual_offset[1]) * scale), visual_size[0] * scale, visual_size[1] * scale}, {active_sprite[0], active_sprite[1], 1, 1, 4, 8}, window_size, 0.36 + fmod(position[1] - 0.125, 16) * 0.01125);
    }
...
int main() {
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        if(frames % 100 == 0) {
            ...
            frames = 0;
            std::vector<tile_ID> tiles_stepped_on = get_blocks_around({player.walk_box[0] + player.position[0], player.walk_box[1] + player.position[1], player.walk_box[2], player.walk_box[3]});
            for(tile_ID t : tiles_stepped_on) {
