...
std::array<unsigned char, 4> get_pixel2(unsigned char* data, std::array<int, 2> pixel, int image_width) {
    int pixel_index = pixel[0] + pixel[1] * image_width;
    return {data[pixel_index * 4], data[pixel_index * 4 + 1], data[pixel_index * 4 + 2], 0xFF};
}
...
struct Worldgen {
    ...
    std::array<unsigned char, 4> retrieve2(std::array<int, 2> location, std::array<int, 2> map_size) {
        double elevation = pfac1.retrieve(location[0], location[1], map_size[0], map_size[1]) * 12;
        ...
        std::array<unsigned char, 4> biome = get_pixel2(biome_map_terra, {floor(elevation) * 12 + floor(humidity), floor(temperature)}, 144);
        if(elevation > 7 && elevation <= 7.03 && biome[0] != 0xE5U && biome[0] != 0xF4U && biome[0] != 0x2DU && temperature >= 3) biome = {0xFFU, 0xFBU, 0xB3U, 0xFFU};
        //printf("%u ", biome);
        return biome;
    }
};

std::array<int, 2> get_floor_tile(unsigned int biome, Worldgen* worldgen) {
    double rand;
    int block_type;
    int texture;
    switch(biome) {
...
std::array<int, 2> get_object_tile(unsigned int biome, Worldgen* worldgen) {
    double rand;
    switch(biome) {
