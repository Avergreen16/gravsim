...
std::array<int, 256> generate_chunk(Worldgen* worldgen, sf::Vector2u top_left, bool is_overlay) {
    ...
    for(int x = 0; x < chunk_size.x; x++) {
        for(int y = 0; y < chunk_size.y; y++) {
            ...
            if(is_overlay) chunk[x + y * chunk_size.x] = get_floor_tile_overlay(worldgen->biomegen->get_biome(top_left.x + x, top_left.y + y, 39200, 19600), worldgen, top_left.x + x, top_left.y + y);
            else chunk[x + y * chunk_size.x] = get_floor_tile(worldgen->biomegen->get_biome(top_left.x + x, top_left.y + y, 39200, 19600), worldgen, top_left.x + x, top_left.y + y);
        }
    }
    return chunk;
}
...
int main() {
    ...
    tilegen.set_parameters(0.754254, 21.93727, 50, 50, 1, 1, 0, 0.5, true, true);
    tilegen.generate_vectors();

    Worldgen worldgen(&pbg, &tilegen, sf::Vector2<double>(392, 392));
        ...
        while(window.pollEvent(event_)) {
            switch(event_.type) {
                ...
                case sf::Event::MouseWheelScrolled:
                    delta = event_.mouseWheelScroll.delta;
                    scale *= pow(1.2, delta);
            ...
            if(rect_collision(sf::Rect<double>(p.second), sf::Rect<double>(camera_pos - sf::Vector2<double>(window_size) / (scale * 2), sf::Vector2<double>(window_size) / (scale)))) {
                for(int x = 0; x < chunk_size.x; x++) {
                    for(int y = 0; y < chunk_size.y; y++) {
                        ...
                        sf::Vector2<double> position(uncentered_x + double(window.getSize().x) / double(2), uncentered_y + double(window.getSize().y) / double(2));
            ...
            if(rect_collision(sf::Rect<double>(p.second), sf::Rect<double>(camera_pos - sf::Vector2<double>(window_size) / (scale * 2), sf::Vector2<double>(window_size) / (scale)))) {
                for(int x = 0; x < chunk_size.x; x++) {
                    for(int y = 0; y < chunk_size.y; y++) {
                        double uncentered_x = (x + p.second.left - camera_pos.x) * scale;
                        double uncentered_y = (y + p.second.top - camera_pos.y) * scale;
                        sf::Vector2<double> position(uncentered_x + double(window.getSize().x) / 2, uncentered_y + double(window.getSize().y) / 2);
                        tile_sprite.setPosition(position.x, position.y);
                        int tile_ID = p.first[x + y * 16];
                        tile_sprite.setTextureRect(sf::IntRect((tile_ID % 8) * 16, int(tile_ID / 8) * 16, 16, 16));
                        tile_sprite.setScale(scale / 16, scale / 16);
