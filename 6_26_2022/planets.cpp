...
void main() {
    ...
    float width_at_height = sqrt(1 - internal_pos.y * internal_pos.y / (1.0 - eccentricity));
...
struct Planemo {
    ...
    void set_oblate(double eccentricity) {
        double major_axis = cbrt(radius * radius * radius / (1 - eccentricity));
        radii = {major_axis, major_axis * (1 - eccentricity)};
        oblate = true;
    }
    ...
    void render(int window_width, int window_height, clock_t time) {
        ...
        if(scaled_diameter < 5) {
            ...
            if(oblate) {
                bounding_box = {(position[0] - radii[0] - camera_pos[0]) * scale, (position[1] - radii[1] - camera_pos[1]) * scale, radii[0] * 2 * scale, radii[1] * 2 * scale};
            } else {
...
int main() {
    ...
    for(int i = 0; i < 8; i++) planemo_v.push_back(new Planemo);
    for(int i = 0; i < 1; i++) barycenter_v.push_back(new Barycenter);
    std::vector<int> order_v;
    
    planemo_v[0]->construct(86000, 3.9, 0, false, 0);
    planemo_v[0]->set_shader(COLOR, color_shader, 0, NULL, {1.0f, 0.4f, 0.0f});
    planemo_v[0]->position = {0, 0};
    order_v.push_back(PLANEMO);
    order_v.push_back(0);

    planemo_v[1]->construct(20, 5.1, 60, false, 45);
    planemo_v[1]->set_shader(TEXTURE_SHADOW, texture_shadow_shader, planet_texture, planemo_v[0], {0.2f, 0.5f, 1.0f});
    planemo_v[1]->set_orbit(PLANEMO, (void*)planemo_v[0], 13000, 15, false);
    order_v.push_back(PLANEMO);
    order_v.push_back(1);

    planemo_v[2]->construct(9, 4.7, 97, false, 4);
    planemo_v[2]->set_shader(TEXTURE_SHADOW, texture_shadow_shader, planet_texture2, planemo_v[0], {1.0f, 0.8f, 0.3f});
    ...
    planemo_v[3]->construct(204, 0.5, 0, false, 30);
    planemo_v[3]->set_shader(COLOR_SHADOW, color_shadow_shader, 0, planemo_v[0], {0.9f, 0.8f, 1.0f});
    planemo_v[3]->set_orbit(PLANEMO, (void*)planemo_v[0], 1700, 20, false);
    order_v.push_back(PLANEMO);
    order_v.push_back(3);
    //planemo_v[3]->set_oblate(0.25);

    planemo_v[4]->construct(11640, 2.0, 0, false, 0);
    planemo_v[4]->set_shader(COLOR_SHADOW, color_shadow_shader, 0, planemo_v[0], {0.6f, 0.2f, 0.3f});
