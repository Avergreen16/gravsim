        ...
        if(current_activity == PLAY) {
            ...
            if(key == GLFW_KEY_BACKSPACE) {
                if(char_callback_string.size() != 0 && type_pos != 0) {
                    char_callback_string.erase(char_callback_string.begin() + type_pos - 1);
                    --type_pos;
                }
                ...
                if(char_callback_string.size() != 0 && type_pos != 0) --type_pos;
            } else if(key == GLFW_KEY_RIGHT) {
                if(char_callback_string.size() != 0 && type_pos < char_callback_string.size()) ++type_pos;
            }
        }
...
void process_input(std::string input, Entity &player, std::list<std::string> &chat_vector) {
    if(input[0] == '*') {
        // command
        if(strcmp(input.substr(1, 4).data(), "tpm ") == 0) {
            ...
            if(x_val < 0 || x_val >= world_size[0] || y_val < 0 || y_val >= world_size[1]) {
                chat_vector.emplace_front("Invalid parameters. (*tpm)");
            } else {
                player.position = {x_val + 0.5, y_val + 0.25};
                chat_vector.emplace_front("Teleported " + player.name.str + "\\c000 to coordinates: " + std::to_string(x_val) + " " + std::to_string(y_val));
            }
        } else {
            chat_vector.emplace_front("Invalid command.");
        }
    } else {
        // chat
        if(input.size() != 0) chat_vector.emplace_front(player.name.str + "\\c000: " + input);
        if(chat_vector.size() >= 10) chat_vector.pop_back();
    }
}
...
int main() {
    ...
    player.construct({23472 + 0.5, 10384 + 0.5}, player_spritesheet, "\\c82fAvergreen");
    ...
    std::list<std::string> chat_vector;
    ...
    while(!quit_game) {
        ...
        if(enter_pressed) {
            process_input(char_callback_string, player, chat_vector);
            char_callback_string = "";
            current_activity = PLAY;
            enter_pressed = false;
            type_pos = 0;
        ...
        render_text(text_shader_program, text_img, text_img_info, {width, height}, {float(floor(width / 2) - 10 - (version.get_len() * 2)), float(floor(height / 2) - 64)}, version.str, 2, 1000);
        render_text(text_shader_program, text_img, text_img_info, {width, height}, {float(floor(width / 2) - 10 - ((hex_coords.size() - 4) * 7 - 3) * 2), float(floor(height / 2) - 92)}, hex_coords, 2, 500);
        render_text(text_shader_program, text_img, text_img_info, {width, height}, {float(floor(width / 2) - 10 - (dec_coords.size() * 7 - 3) * 2), float(floor(height / 2) - 118)}, dec_coords, 2, 500);
        render_text_type(text_shader_program, text_img, text_img_info, {width, height}, {float(-floor(width / 2) + 10), float(floor(height / 2) - 36)}, char_callback_string, 2, (current_activity == CHAT) ? type_pos : -1);
        ...
        int text_y_pos = floor(height / 2) - 64;
        for(std::string line : chat_vector) {
            int lines_passed = render_text(text_shader_program, text_img, text_img_info, {width, height}, {float(-floor(width / 2) + 10), float(text_y_pos)}, line, 2, 800);
