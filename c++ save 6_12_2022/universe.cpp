...
int main() {
    ...
    sf::Texture map_2;
    map_2.loadFromFile("map101x.png");
    ...
    ss1.spaceobjects[0]->star(&star_shader, ss1.spaceobjects[0], NULL, false, 187000, pow((double)187000 / 200000 + 0.1 * rng.retrieve(), -1.3) * 1.4, sf::Vector2<double>(0, 0), sf::Vector2<double>(0, 0), sf::Vector3f(1.0f, 0.7f, 0.2f), "G-class star", 2, false);
    ss1.stars.push_back(ss1.spaceobjects[0]);

    add_satellite(&texture_shader, ss1.spaceobjects[1], ss1.spaceobjects[0], false, ss1.spaceobjects[0], 19300, -70, 20, 5.1, sf::Vector3f(0.2f, 0.9f, 0.7f), "wet terrestrial with life", false);
    ss1.spaceobjects[1]->tex = &map_;
    ss1.spaceobjects[1]->clock = &clock;
        ...
        switch(active_view_scale) {
            case STAR:
                ...
                switch(active_view_setting) {
                    ...
                    case SURFACE:
                        map_sprite.setScale(scale * 20, scale * 20);
                        map_sprite.setPosition(-camera_pos.x * scale + window_width / 2, -camera_pos.y * scale + window_height / 2);

                        map_2sprite.setScale(scale, scale);
                        map_2sprite.setPosition((-camera_pos.x + 657 * 20) * scale + window_width / 2, -(camera_pos.y - 171 * 20) * scale + window_height / 2);
