...
void generate_stars(std::array<int, 2> map_size, unsigned char* map, int star_distance, std::vector<Planemo*>* planemo_v, std::vector<int>* order_v, unsigned int shader) {
    RNG rng;
    rng.set_seed(0.3763097, 67.0983648);
        ...
        for(int x = 0; x < map_size[0]; x++) {
            if(map[(x + y * map_size[0]) * 4 + 3] == 0xff) {
                ...
                switch(green_value) {
                    ...
                    case 0xe7:
                        remaining_star_amount = 0.4;
                        break;

                    case 0x3b:
                        remaining_star_amount = 0.25;
                        break;
                ...
                if(rng.retrieve() <= remaining_star_amount) {
                    double x_pos = (x + 0.15 + rng.retrieve() * 0.7) * star_distance;
                    double y_pos = (y + 0.15 + rng.retrieve() * 0.7) * star_distance;
...
int main() {
    ...
    planemo_v[189]->construct(189000, 1.5, 0, false, 0, true);
    planemo_v[189]->color = {1.0f, 0.8f, 0.0f};

    planemo_v.push_back(new Planemo);
    planemo_v[634]->construct(20, 5.1, 60, false, -15, true);
    planemo_v[634]->set_shader(TEXTURE_SHADOW, texture_shadow_shader, tex_terra, planemo_v[0], {0.2f, 0.5f, 1.0f});
    planemo_v[634]->set_orbit(PLANEMO, (void*)planemo_v[189], 43000, 15, false);
    order_v.push_back(PLANEMO);
    order_v.push_back(634);
