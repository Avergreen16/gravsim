...
double arcsin_approx(double x) {
    return x + 0.5 * (x * x * x / 3) + 0.375 * (x*x*x*x*x / 5) + 0.3125 * (x*x*x*x*x*x*x / 7) + 0.2734375 * (x*x*x*x*x*x*x*x*x / 9);
}
...
void PlanetTerrainGenerator::assign_colors(SDL_Surface* surface) {
    std::map<std::string, Uint32> biome_lookup_table = {
        {"shadow", SDL_MapRGBA(surface->format, 100, 20, 20, 255)},
        {"light", SDL_MapRGBA(surface->format, 255, 50, 50, 255)},
    ...
    for(int i = 0; i < v_fac.point_map_width * v_fac.point_map_height; i++) {
        v_Point* pt = &v_fac.point_map.at(i);
        pt->color = biome_lookup_table.at(get_biome(p_fac_elev.pullvalue(pt->coordinates[0], pt->coordinates[1], 100, 100)));
    }
}

void PlanetTerrainGenerator::draw_planet(SDL_Surface* surface) {
    Uint32 color;
    for(int y = 0; y < surface->h; y++) {
        double py = ((double)y + 0.5) / surface->h * 2 - 1;
        double width_at_height = std::sqrt(1 - py * py);
        double py2 = arcsin_approx(py) * 2/M_PI;
        for(int x = round(surface->w / 2 - surface->w / 2 * width_at_height); x < round(surface->w / 2 + surface->w / 2 * width_at_height); x++) {
            double px = ((double)x + 0.5) / surface->w * 2 - 1;
            //if(px * px + py * py > 1) {
            //    continue;
            //}
            double px2 = arcsin_approx(px / width_at_height) * 2/M_PI;
            if(px2 > 0.4) {
                color = SDL_MapRGBA(surface->format, 100, 20, 20, 255);//biome_lookup_table.at("shadow");
            //} else if(px2 > -0.3) {
            //    color = SDL_MapRGBA(surface->format, 150, 30, 30, 255);
            } else {
                color = SDL_MapRGBA(surface->format, 255, 50, 50, 255);//biome_lookup_table.at("light");
            }
...
void PlanetTerrainGenerator::draw_planet_blue(SDL_Surface* surface) {
    Uint32 color;
    for(int y = 0; y < surface->h; y++) {
        double py = ((double)y + 0.5) / surface->h * 2 - 1;
        double width_at_height = std::sqrt(1 - py * py);
        ...
        for(int x = round(surface->w / 2 - surface->w / 2 * width_at_height); x < round(surface->w / 2 + surface->w / 2 * width_at_height); x++) {
            ...
            double px2 = arcsin_approx(px / width_at_height) * 2/M_PI;
            if(px2 > 0.4) {
                color = SDL_MapRGBA(surface->format, 20, 20, 100, 255);
            } else {
                color = SDL_MapRGBA(surface->format, 50, 50, 255, 255);
            }
            ((Uint32*)surface->pixels)[x + y * surface->w] = color;
        }
    }
