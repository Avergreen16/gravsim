...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if(action == GLFW_PRESS) {
       ...
       if(!key_down_array[GLFW_KEY_LEFT_SHIFT] && key_down_array.find(key) != key_down_array.end()) key_down_array[key] = false;
    }
}
...
int main() {
    ...
    while(!quit_game) {
        ...
        render_text(square_shader_program, text_img, text_img_info, {width, height}, {float(floor(width / 2) - 229), float(floor(height / 2) - 46)}, hex_coords, 3, 500);
        render_text(square_shader_program, text_img, text_img_info, {width, height}, {float(floor(width / 2) - 229), float(floor(height / 2) - 85)}, dec_coords, 3, 500);
        render_text(square_shader_program, text_img, text_img_info, {width, height}, {float(-floor(width / 2) + 10), float(floor(height / 2) - 46)}, "\\x0123456789abcdef", 3, 500);
        render_text(square_shader_program, text_img, text_img_info, {width, height}, {float(-floor(width / 2) + 10), float(floor(height / 2) - 85)}, "0123456789", 3, 500);
        render_text(square_shader_program, text_img, text_img_info, {width, height}, {float(-floor(width / 2) + 10), float(floor(height / 2) - 124)}, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijhlmnopqrstuvwxyz", 3, 500);

        int text_scale = std::max(int(round(0.03125 * scale)), 1);
        render_text(square_shader_program, text_img, text_img_info, {width, height}, {float(-label.get_len() / 2 * text_scale), float(0.875 * scale)}, label.str, text_scale, 1000);
